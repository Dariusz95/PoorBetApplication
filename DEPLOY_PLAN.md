...
**Uwaga (Etap 10):** to jest opis jednorazowego, ręcznego pierwszego postawienia serwera. Każda kolejna aktualizacja `docker-compose.yml`/`docker-compose.prod.yml` (i sam deploy nowego `IMAGE_TAG`) odbywa się już automatycznie przez CD — patrz `OPERATIONS.md`. Ręczny `scp`/SSH z tego etapu zostaje jedynie jako fallback, gdyby automatyzacja kiedyś zawiodła.
...
Na tym etapie appka działa lokalnie na serwerze (`curl http://127.0.0.1:8080` powinno odpowiadać) — jeszcze nic nie jest wystawione publicznie z TLS. **Uwaga:** `frontend` publikuje się na `127.0.0.1:8080`, nie na porcie 80 — port 80/443 na hoście jest zarezerwowany wyłącznie dla Nginxa z Etapu 8.4.
...
**Od Etapu 10 ten krok jest zautomatyzowany.** Cały update to jedna komenda lokalnie:

```bash
scripts/release.sh          # albo: scripts/release.sh vX.Y.Z
```

Wypchnięcie tagu uruchamia CI (testy → build i publikacja 8 obrazów do GHCR → automatyczny deploy przez SSH na VPS, job `deploy` w `publish-images.yml`) — bez ręcznego SSH. Szczegóły mechanizmu i jednorazowa konfiguracja serwera pod to CD: `OPERATIONS.md`.

Ręczna ścieżka (fallback, gdyby CD zawiodło albo do awaryjnego redeployu starszej wersji):

```bash
# lokalnie: git tag vX.Y.Z && git push origin vX.Y.Z   → CI buduje i publikuje do GHCR
# na serwerze:
IMAGE_TAG=vX.Y.Z ./scripts/deploy.sh
# albo bez skryptu, wprost:
...
## Etap 10 — Automatyzacja deployu (CD) i zarządzanie serwerem ✅ ZROBIONE

Etap 8 opisywał pierwszy, w pełni ręczny deploy — SSH, ręczny `scp`, ręczny `export IMAGE_TAG`, ręczny `pull`/`up -d` przy każdej aktualizacji. Ten etap to na to odpowiedź: automatyzacja tego, co się powtarza przy każdym wydaniu.

Pełny opis mechanizmu (co, jak i **dlaczego** tak zaprojektowane — pinning `known_hosts`, ryzyko grupy `docker`, `docker compose --wait`, jednorazowa konfiguracja serwera) jest w osobnym pliku **`OPERATIONS.md`**, żeby nie rozdymać tego dokumentu — ten etap jest tylko krótkim podsumowaniem:

- [x] `scripts/deploy.sh` — uruchamiane na serwerze (przez CD albo ręcznie jako fallback), zastępuje ręczny `pull`/`up -d` z Etapu 8.2/8.5.
- [x] `scripts/release.sh` — uruchamiane lokalnie przez dewelopera, zastępuje ręczny `git tag && git push`.
- [x] Nowy job `deploy` w `.github/workflows/publish-images.yml` — po opublikowaniu wszystkich 8 obrazów, CI samo łączy się po SSH na VPS, kopiuje świeże `docker-compose.yml`/`docker-compose.prod.yml`/`scripts/deploy.sh` i odpala deploy.
- [x] Poprawiony `Makefile` — `prod`/`prod-pull` faktycznie ciągną obrazy z GHCR (wcześniej po cichu budowały lokalnie z `docker-compose.prod-build.yml`, myląca nazwa `COMPOSE_PROD_BUILD` vs nieużywany `COMPOSE_PROD`).
- [ ] **Świadomie poza zakresem:** selektywny rebuild tylko zmienionych obrazów w CI (Etap 6 nadal buduje wszystkich 8 na każdy tag — decyzja, nie przeoczenie) oraz automatyczne backupy/rollback (zostają jako ręczny dług, Etap 9 wyżej).
- [ ] **Wymaga jednorazowej, ręcznej konfiguracji serwera** (dedykowany user `deploy`, klucz SSH, 6 sekretów w GitHub) — opisane krok po kroku w `OPERATIONS.md`, nie zrobione automatycznie w ramach tej zmiany.
